...
if(isset($_POST['action_taken'])){
    if(strtoupper($_POST['action_taken'])=="OUT"){
         $queryString = "UPDATE tbl_timesheet SET newtimeout = '" .$current_time. "' WHERE userId = '" .$_POST['userId']. "' AND id = '" .$_POST['timesheetId']. "' AND newtimeout='0000-00-00 00:00:00'";
        // echo $queryString;
    }
    ...
    if(strtoupper($_POST['action_taken'])=="LATESTTIME"){
        ...
        //$queryString = "INSERT INTO tbl_ips(date, timesheetid, myip, userid)VALUES('" .$current_time. "', '" .$_POST['timesheetId']. "', '".$ip."', '" .$_POST['userId']. "')";
        //$result = mysqli_query($app->connect(),$queryString) or die(mysqli_connect_error());
        
        $queryString = "SELECT * FROM tbl_ips WHERE timesheet_id = '".$_POST['timesheetId']."' AND userid = '".$_POST['userId']."'" ; 
        ...
        if(mysqli_num_rows($resulte)==0){
            ...
            if(isset($_POST['version'])){ 
                $keyadd=", tracker_version";
                $valadd=", '".$_POST['version']."'";
            }
            $queryString = "INSERT INTO tbl_ips(date, timesheet_id, myip, userid, location ".$keyadd.")VALUES('" .$current_time. "', '" .$_POST['timesheetId']. "', '".$ip."', '" .$_POST['userId']. "', '".$location."' ".$valadd.")";
            $result = mysqli_query($app->connect(),$queryString) or die(mysqli_connect_error());
        }
    ...
    $result = mysqli_query($app->connect(),$queryString) or die(mysqli_connect_error());
    if(strtoupper($_POST['action_taken'])=="LATESTTIME"){
        $queryString = "SELECT shift_start, shift_end, shift_type FROM tbl_employee WHERE id = '" .$_POST['userId']. "'" ; 
        $resulte = mysqli_query($app->connect(),$queryString) or die(mysqli_connect_error());
        $theEmp = array();
       while($edata = mysqli_fetch_assoc($resulte)){
        ...
        if($theEmp[0]['shift_type']=="Day"){
            $follow_date = date('Y-m-d', time());
        }else{
            $follow_date = date('Y-m-d', strtotime($current_date." -1 day"));
        }
        $shift_start = date("Y-m-d H:i:s", strtotime($follow_date." ".$theEmp[0]['shift_start']." -1 hour"));
        $shift_end = date("Y-m-d H:i:s", strtotime($shift_start."  +16 hour")); 
        //date("Y-m-d H:i:s", strtotime($follow_date." ".$theEmp[0]['shift_start']."  +14 hour"));
        ...
        //echo $theEmp[0]['shift_start']." ".$shift_start." ".$shift_end."   ";
        ...
        $queryString = "SELECT * FROM tbl_timesheet WHERE userId = '" .$_POST['userId']. "' AND ( id = '".$_POST['timesheetId']."' OR (newtimein>='".$shift_start."' AND (newtimeout<= '".$shift_end."' OR newlastmin<='".$shift_end."'))) " ; 
                                                                                                             //AND newtimein>='".$dfrom."' AND (newtimeout<='".$dto."' OR newlastmin<='".$dto."') 
        //echo $queryString."   ";
        $result = mysqli_query($app->connect(),$queryString) or die(mysqli_connect_error());
        $responseData=array();
        $time1 = "00:00:00";
