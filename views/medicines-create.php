...
if(isset($_GET['id'])){
    $action = "Update";
    $rqdata = array("model"=>"medicines", "condition"=>" WHERE id = '".$_GET['id']."'");
    $department = $app->getRecord2($rqdata);
    $rvalue = $department['data'][0];
    $brands = json_decode($rvalue["brands"]);
    $diseases = json_decode($rvalue["diseases"]);
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <div class="pull-right" style="margin-top: -25px;">
                            <a href="?page=medicines"><label class="btn btn-xs btn-info">Medicine List</label></a> 
                        </div>
                    </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-8">
                                ...
                                <label>Brands</label>
                                    ...
                                    <select class="form-control select2 select2-hidden-accessible" multiple="" data-placeholder="Select brands" name="brands[]" style="width: 100%;" tabindex="-1" aria-hidden="true" required>
                                        <?php
                                         foreach ($rBrands as $key => $value) {
                                              $act="";
                                             if(isset($_GET['id']) && in_array($key, $brands)){  $act="selected"; }
                                ...
                                <label>Diseases</label>
                                    <select class="form-control select2 select2-hidden-accessible" multiple="" data-placeholder="Select Diseases" name="diseases[]" style="width: 100%;" tabindex="-1" aria-hidden="true" required>
                                        <?php
                                         foreach ($rDiseases as $key => $value) { $act="";
                                             if(isset($_GET['id']) && in_array($key, $diseases)){  $act="selected"; }
                                             echo "<option value='".$key."' ".$act.">".$value['name']."</option>";
                                         }
                                        ?>
                                    </select>   
